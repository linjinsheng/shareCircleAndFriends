...
/** 扫描内容的 W 值 */
#define scanBorderW 260
/** 扫描内容的 x 值 */
#define scanBorderX ([UIScreen mainScreen].bounds.size.width-260)/2
/** 扫描内容的 Y 值 */
#define scanBorderY 151
...
- (void)initialization {
    ...
    _cornerColor = [UIColor colorWithRed:60/255.0f green:107/255.0 blue:240/255.0 alpha:1.0];
    _cornerWidth = 3.0;
    _backgroundAlpha = 0.5;
    _animationTimeInterval = 0.02;
    _scanningImageName = @"SGQRCode.bundle/me_code_line";
}
...
-(UILabel*)tipLabel
{
    if (!_tipLabel) {
        _tipLabel=({
            ...
            label.text=NSLocalizedString(@"扫描二维码提示语", nil);
...
- (void)addTimer {
    ...
    [_tipLabel setFrame:CGRectMake(0, 0, _tipView.frame.size.width, _tipView.frame.size.height)];
    ...
    if (self.scanningAnimationStyle == ScanningAnimationStyleGrid) {
        ...
        _scanningline.frame = CGRectMake(scanninglineX, scanninglineY, scanninglineW, _scanningline.image.size.height);
    }
...
- (UIImageView *)scanningline {
    if (!_scanningline) {
        ...
        UIImage *image= [UIImage imageNamed:self.scanningImageName];
